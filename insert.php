...
<body style="background-color: #99ccff" onload="changelan(0)"> 
...
<div id="projects-eng" style="margin-top: 150px; margin-bottom: 50px">
 <section class="testimonials_area p_120" style="background-color: #ffffcc">
    <div class="container">
...
//database part
	...
	if(mysqli_select_db($conn,'uploads')){
		...
		if(!$conn->query("DESCRIBE alevel")){
			$create_tb="create table alevel (number int AUTO_INCREMENT PRIMARY KEY, up_date date not null, s_name varchar(200) not null,text_file varchar(250) not null,attach_file varchar(250),email varchar(250));";
...
//enter data
$sql="insert into alevel ( up_date , s_name ,text_file ,attach_file ,email ) values ($date,'$s_name','$filename','$attach_file','$email');";
...
<!-- ==== Form Area === -->
<div name="q-form" class="col-lg-8" style="margin-bottom: 50px; background-color: #ff33ff; padding: 20px;width: 100%; margin-left: 15%">
...
<script>
	...
	function changelan(x){
		if(x==0){
		/*hide image area while select file*/
		document.getElementById('uploadimage').style.display="none";
			...
			document.getElementById("list").placeholder="Subject";
			...
			document.getElementById("ata-im").innerHTML="Attach Image";
		...
		if(x==1){
			...
			document.getElementById("ata-im").innerHTML="පින්තූරයක් අමුනන්න ";
